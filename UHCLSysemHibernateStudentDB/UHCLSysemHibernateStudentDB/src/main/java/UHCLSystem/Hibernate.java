...
public class Hibernate implements Data{
	...
	public void writeNote(String writer, String course, String content , String datetime) {
		 ...
		 * 
		 * System.out.println("Enter the content for the course note:"); String content1
		 * = input.nextLine();
		 */

		    CourseNote c = new CourseNote(0, writer, course, content, datetime);
		    session.save(c); // Save the new object directly
		    session.getTransaction().commit();
		...
		public ArrayList<CourseNote> getCourseNotes(String courseID) {
			// TODO Auto-generated method stub
			setup();
			Session session = sessionFactory.openSession();
			session.beginTransaction();
			
			String note = "FROM CourseNote N WHERE N.courseID = :courseID";
			Query query = session.createQuery(note);
		...
		public ArrayList<CourseNote> getCourseNotes(String course, String courseId) {
			// TODO Auto-generated method stub
			return null;
		}
